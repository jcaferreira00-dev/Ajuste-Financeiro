...
(function () {
  ...
  const UNIDADE_REF_KLABIN = 4736;     // Dados!E13 - base para "PACK Básico VGBL" (não é PGBL)
  ...
  function calcINSS(baseProventos) {
    ...
    return 988.07; // teto INSS — confirmado em holerite real
  }
  ...
  function salvarCamposFixos() {
    const dados = {};
    FIXED_FIELD_IDS.forEach(id => { dados[id] = document.getElementById(id).value; });
    FIXED_TOGGLE_IDS.forEach(id => { dados[id] = document.getElementById(id).checked; });
  ...
  function carregarCamposFixos() {
    ...
    try {
      ...
      FIXED_FIELD_IDS.forEach(id => {
        if (dados[id] !== undefined && dados[id] !== '') document.getElementById(id).value = dados[id];
      });
      FIXED_TOGGLE_IDS.forEach(id => {
        if (dados[id] !== undefined) document.getElementById(id).checked = dados[id];
      });
    } catch (e) {
      console.warn('Não foi possível carregar os campos fixos salvos do Holerite.', e);
    }
  }
  ...
  function calcular() {
    ...
    // ---- DESCONTOS (exceto IRRF, que depende da Base IR editável) ----
    const inss = calcINSS(totalProventos);

    // "PACK Básico VGBL" não reduz a base do IR (VGBL não é dedutível, diferente de PGBL)
  ...
  function atualizarDecimoTerceiro() {
    ...
    const el = document.getElementById('decimoTerceiroTotal');
  ...
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) tentarFecharModal();
  });
  ...
  document.getElementById('clearForm').addEventListener('click', () => {
    document.querySelectorAll('#holeriteSheet input[type="number"]').forEach(i => i.value = '');
    document.querySelectorAll('#holeriteSheet input[type="checkbox"]').forEach(i => i.checked = false);
